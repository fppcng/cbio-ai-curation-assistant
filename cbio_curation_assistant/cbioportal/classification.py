"""Pure, specification-driven cBioPortal sheet classification.

This module does not resolve specifications or perform network access. Callers
provide the specifications and their source metadata, making classification
deterministic for a given DataFrame and specification sequence.

Scoring preserves the established behavior:
  confidence = (required_hits / total_required) × 70
              + (optional_hits  / total_optional)  × 30
  minimum accepted confidence = 40
"""
...
from cbio_curation_assistant.cbioportal.specs import FormatSpec
...
def classify_sheet(
    df: pd.DataFrame,
    specifications: Sequence[FormatSpec],
    *,
    spec_source: str,
    spec_fetched_at: str,
