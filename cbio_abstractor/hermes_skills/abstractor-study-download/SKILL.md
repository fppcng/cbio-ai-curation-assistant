...
## Workflow
1. Check whether `studies/<PMCID>/data/raw/manifest.json` already exists and still points to files that are present on disk.
...
5. After the run, verify the manifest and the files on disk under `studies/<PMCID>/data/raw/`.
...
- Report the canonical study path: `studies/<PMCID>/data/raw/`
