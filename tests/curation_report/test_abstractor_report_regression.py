...
class AbstractorReportRegressionTest(unittest.TestCase):
    def test_pmc8317046_deterministic_fields_match_gold(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="abstractor_regression_") as tmp_dir:
            ...
            self.assertEqual(Path(result["report_json_path"]), output_json_path.resolve())
            self.assertEqual(Path(result["pdf_path"]), output_pdf_path.resolve())
            self.assertEqual(Path(result["agent_report_json_path"]), agent_report_path.resolve())
