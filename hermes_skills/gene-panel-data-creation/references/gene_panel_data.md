...
### Values
For each sample-profile combination, specify a named gene panel, a supported built-in assay value, or `NA`. Named gene panels must be imported before loading the study data. The built-in values `WXS`, `WGS`, and `WXS/WGS` represent explicitly supported whole-exome, whole-genome, or combined whole-exome/whole-genome profiling and do not require an imported panel definition. Use `NA` only when the available evidence establishes that the sample-profile combination was not profiled. If the profiling status is unresolved, report the missing evidence instead of encoding the uncertainty as `NA`. If the sample is profiled for mutations, make sure it is also in the `_sequenced` case list.
