...
## Workflow
1. Use the initialized study workspace identified by `<study_id>`. The script reloads it from `studies/<study_id>/study_manifest.json` and works in `studies/<study_id>/curated/`.
...
3. Build `studies/<study_id>/curated/minimal_mutations.maf` using the minimal columns required by the local Genome Nexus runner. Add other fields only when directly supported by the source.
...
## Output
Report these study files under `studies/<study_id>/curated/`:
